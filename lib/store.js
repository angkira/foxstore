...
class ProtoStore {
     ...
     */
    select(entityName) {
        return rxjs_1.pipe(operators_1.distinctUntilChanged(), operators_1.takeUntil(this.eventDispatcher.destroy$), operators_1.shareReplay(1))(entityName ?
            this.store$.pipe(operators_1.map(ramda_1.path([entityName])))
    ...
    on(eventName, callbackFn, options) {
        this.eventDispatcher
            .listen(eventName)
            .pipe((options.once ? operators_1.take(1) : operators_1.map(rxjs_1.identity)))
