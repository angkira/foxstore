...
exports.setupStoreEventsFromDecorators = (store, eventScheme = {}) => {
    const effects = Reflect.getMetadata(EFFECT_METAKEY, store)
        || [];
    const reducers = Reflect.getMetadata(REDUCER_METAKEY, store)
        || [];
    const actions = Reflect.getMetadata(ACTION_METAKEY, store)
        || [];
