...
const setupStoreEvents = (eventScheme = {}) => (newInstance) => {
    ...
    const payloadStreams = ramda_1.keys(eventScheme)
        ...
        var _a, _b, _c, _d, _e;
        const logger = (((_a = newInstance.options) === null || _a === void 0 ? void 0 : _a.logOn) && ((_b = newInstance.options) === null || _b === void 0 ? void 0 : _b.logger)) || rxjs_1.noop;
        logger({
            event: eventName,
            payload: payloadObject
        });
        if (eventScheme[eventName].reducers instanceof Array && ((_c = eventScheme[eventName].reducers) === null || _c === void 0 ? void 0 : _c.length)) {
            reducerHandler(payloadObject, state)(eventScheme[eventName].reducers);
        }
        if (eventScheme[eventName].effects instanceof Array && ((_d = eventScheme[eventName].effects) === null || _d === void 0 ? void 0 : _d.length)) {
            effectHandler(payloadObject, state)(eventScheme[eventName].effects);
        }
        if (eventScheme[eventName].actions instanceof Array && ((_e = eventScheme[eventName].actions) === null || _e === void 0 ? void 0 : _e.length)) {
...
function metaGetEntityPayload({ eventDispatcher, store$ }) {
    return (eventName) => eventDispatcher
        .listen(eventName)
        .pipe(
    // tap(x => console.log(x)), // TODO: create Log-plugin to log events. ReduxTools - maybe
    operators_1.shareReplay(1), operators_1.mergeMap((event) => (event.async ?
        event.payload
        : rxjs_1.of(event.payload))
        .pipe(operators_1.take(1))), operators_1.withLatestFrom(store$.asObservable()), operators_1.share());
}
/**
 * Handler for reducer
 * @param instance
...
function effectMetaHandler(instance) {
    return (payload, state) => (effects) => effects.forEach(effect => effect.effect.call(instance, payload, state));
}
/**
 * Handler for Action
 * @param instance
...
function actionMetaHandler(instance) {
    return (payload, state) => (actions) => actions.forEach(action => {
        ...
        const patch = (payload) => {
            var _a;
            return instance.patch(types_1.simplyReducer((_a = action.options) === null || _a === void 0 ? void 0 : _a.writeAs)
                .call(instance, payload, state));
        };
        if ((_a = action.options) === null || _a === void 0 ? void 0 : _a.writeAs) {
            rxjs_1.isObservable(result.payload) ?
                result.payload.pipe(operators_1.take(1)).subscribe(patch)
