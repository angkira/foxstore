...
function Action(eventName, options, outputEventName) {
    return function (store, propertyKey, descriptor) {
        ...
        if (options === null || options === void 0 ? void 0 : options.writeAs) {
            if (outputEventName) {
                Reducer(outputEventName)(store, `${propertyKey}writeAs`, { value: (payload) => ({ [options.writeAs]: payload }) });
            }
            else {
                console.error('You did not pass outputEventName for Action ' + propertyKey);
            }
        }
